...
function getAddressParts(miner) {
    ...
    if (nameOffset !== -1) {
        ...
        workerName = cnUtil.cleanupSpecialChars(miner.substr(nameOffset + 1));
    }
    else {
        address = miner;
    }
    ...
    async.parallel({
        pool: function(callback){
            redisClient.multi(redisCommands).exec(function(error, replies){
                ...
                for (var miner in minersHashrate){
                    ...
                    minersHashrate[miner] = Math.round(minersHashrate[miner] / global.config.api.hashrateWindow);
                ...
                if (replies[5]){
                    for (var miner in replies[5]){
                        ...
                        if (global.config.poolServer.slushMining.enabled) {
                            roundHashes = parseInt(replies[5][miner]) / Math.pow(Math.E, ((data.lastBlockFound - dateNowSeconds) / global.config.poolServer.slushMining.weight)); //TODO: Abstract: If something different than lastBlockfound is used for scoreTime, this needs change.
                        }
                        else {
        ...
        config: function(callback){
            callback(null, {
                ...
                blockTime: global.config.poolServer.slushMining.blockTime,
                slushMiningEnabled: global.config.poolServer.slushMining.enabled,
                weight: global.config.poolServer.slushMining.weight,
                priceSource: global.config.prices ? global.config.prices.source : 'cryptonator',
                priceCurrency: global.config.prices ? global.config.prices.currency : 'USD',
                paymentIdSeparator: global.config.poolServer.paymentId && global.config.poolServer.paymentId.addressSeparator ? global.config.poolServer.paymentId.addressSeparator : ".",
                ...
                sendEmails: global.config.email ? global.config.email.enabled : false
            });
        },
...
function broadcastWorkerStats(address, destinations) {
    var redisCommands = [
        ...
        ['keys', global.config.coin + ':unique_workers:' + address + '+*']
    ];
    redisClient.multi(redisCommands).exec(function(error, replies){
        ...
        charts.getUserChartsData(address, paymentsData, function(error, chartsData) {
            ...
            for (var i in workersData){
                redisCommands.push(['hgetall', global.config.coin + ':unique_workers:' + address + '+' + workersData[i].name]);
            }
            redisClient.multi(redisCommands).exec(function(error, replies){
                for (var i in workersData) {
...
function handleMinerStats(urlParts, response){
    ...
    if (longpoll){
        ...
        redisClient.exists(global.config.coin + ':workers:' + address, function(error, result){
            if (!result){
...
function minerSeenWithIPForAddress(address, ip, callback) {
    ...
    redisClient.sismember([global.config.coin + ':workers_ip:' + address, ip], function(error, result) {
...
function handleSetMinerPayoutLevel(urlParts, response){
    ...
    var minLevel = global.config.payments.minPayment / global.config.coinUnits;
    ...
    minerSeenWithIPForAddress(address, ip, function (error, found) {
        ...
        var payoutLevel = level * global.config.payments.denomination;
        redisClient.hset(global.config.coin + ':workers:' + address, 'minPayoutLevel', payoutLevel, function(error, value){
            if (error){
...
function handleGetMinerPayoutLevel(urlParts, response){
    ...
    redisClient.hget(global.config.coin + ':workers:' + address, 'minPayoutLevel', function(error, value){
        if (error){
        ...
        var currentLevel = value / global.config.payments.denomination;
...
function handleSetMinerNotifications(urlParts, response){
    ...
    minerSeenWithIPForAddress(address, ip, function (error, found) {
        ...
        if (action === "enable") {
            ...
            redisClient.hset(global.config.coin + ':notifications', address, email, function(error, value){
                if (error){
            ...
            redisClient.hdel(global.config.coin + ':notifications', address, function(error, value){
                if (error){
...
function handleGetMinerNotifications(urlParts, response){
    ...
    redisClient.hget(global.config.coin + ':notifications', address, function(error, value){
        if (error){
...
function handleTopMiners(response) {
    async.waterfall([
        function(callback) {
            redisClient.keys(global.config.coin + ':workers:*', callback);
        },
...
function authorize(request, response){
    ...
    if(global.config.api.trustProxyIP && request.headers['x-forwarded-for']){
...
function handleAdminPorts(response){
    async.waterfall([
        function(callback) {
            redisClient.keys(global.config.coin + ':ports:*', callback);
        },
...
server.listen(global.config.api.port, global.config.api.bindIp, function(){
    log('info', logSystem, 'API started & listening on port %d', [global.config.api.port]);
});

// Start API on SSL port
var ssl_server = null;
if (global.config.api.ssl){
    if (!global.config.api.sslCert) {
        log('error', logSystem, 'Could not start API listening on port %d (SSL): SSL certificate not configured', [global.config.api.sslPort]);
    } else if (!global.config.api.sslKey) {
        log('error', logSystem, 'Could not start API listening on port %d (SSL): SSL key not configured', [global.config.api.sslPort]);
    } else if (!global.config.api.sslCA) {
        log('error', logSystem, 'Could not start API listening on port %d (SSL): SSL certificate authority not configured', [global.config.api.sslPort]);
    } else if (!fs.existsSync(global.config.api.sslCert)) {
        log('error', logSystem, 'Could not start API listening on port %d (SSL): SSL certificate file not found (configuration error)', [global.config.api.sslPort]);
    } else if (!fs.existsSync(global.config.api.sslKey)) {
        log('error', logSystem, 'Could not start API listening on port %d (SSL): SSL key file not found (configuration error)', [global.config.api.sslPort]);
    } else if (!fs.existsSync(global.config.api.sslCA)) {
        log('error', logSystem, 'Could not start API listening on port %d (SSL): SSL certificate authority file not found (configuration error)', [global.config.api.sslPort]);
    } else {
        var options = {
...
if (global.config.api.ssl && ssl_server){
    ssl_server.listen(global.config.api.sslPort, global.config.api.bindIp, function(){
        log('info', logSystem, 'API started & listening on port %d (SSL)', [global.config.api.sslPort]);
    });
}
