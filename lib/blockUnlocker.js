...
function runInterval(){
    async.waterfall([
        ...
        function(callback){
            redisClient.zrange(global.config.coin + ':blocks:candidates', 0, -1, 'WITHSCORES', function(error, results){
                if (error){
        ...
        function(blocks, callback){
            ...
            blocks.forEach(function(block){
                ...
                // Send notifications
                var blockTime = dateFormat(new Date(parseInt(block.time) * 1000), 'yyyy-mm-dd HH:MM:ss Z');
                var blockReward = getReadableCoins(block.reward, 4, false);
                var blockEffort = Math.round(block.shares / block.difficulty * 100) + '%';
                ...
                var message = 'Block ' + block.height + ' with ' + blockReward + ' was mined on ' + blockTime + ' with ' + blockEffort + ' effort.';
