...
var cleanupInterval = global.config.redis.cleanupInterval && global.config.redis.cleanupInterval > 0 ? global.config.redis.cleanupInterval : 15;
...
function getCoinPrice(callback) {
    var source = global.config.prices.source;
    var currency = global.config.prices.currency;
    ...
    if (source === 'cryptonator'){
        ...
        }, '/api/ticker/' + global.config.symbol.toLowerCase() + '-' + currency.toLowerCase());
    }
    ...
    // TradeOgre
        ...
        }, '/api/v1/ticker/BTC-' + global.config.symbol.toUpperCase());
    }
}
