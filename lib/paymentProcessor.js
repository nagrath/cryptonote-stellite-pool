...
if (!global.config.poolServer.paymentId) global.config.poolServer.paymentId = {};
if (!global.config.poolServer.paymentId.addressSeparator) global.config.poolServer.paymentId.addressSeparator = ".";
if (!global.config.payments.priority) global.config.payments.priority = 0;

function getReadableCoins(coins, digits, withoutSymbol){
    var amount = (parseInt(coins || 0) / global.config.coinUnits).toFixed(digits || global.config.coinUnits.toString().length - 1);
    return amount + (withoutSymbol ? '' : (' ' + global.config.symbol));
}
        ...
        function(keys, balances, callback){
            ...
            redisClient.multi(redisCommands).exec(function(error, replies){
                ...
                for (var i = 0; i < replies.length; i++){
                    ...
                    var defaultLevel = global.config.payments.minPayment;
        ...
        function(balances, minPayoutLevel, callback){
            ...
            for (var worker in balances){
                ...
                if (balance >= minPayoutLevel[worker]){
                    ...
                    if (global.config.payments.dynamicTransferFee && global.config.payments.minerPayFee){
                        payout -= global.config.payments.transferFee;
                    }
            ...
            for (var worker in payments){
                ...
                var addr = worker.split(global.config.poolServer.paymentId.addressSeparator);
                ...
                if(global.config.payments.dynamicTransferFee && global.config.payments.minerPayFee){
                    transferCommands[commandIndex].redis.push(['hincrby', global.config.coin + ':workers:' + worker, 'balance', -global.config.payments.transferFee]);
                }
                ...
                if (global.config.payments.dynamicTransferFee){
                    transferCommands[commandIndex].rpc.fee = global.config.payments.transferFee * addresses;
                }

                if (addresses >= global.config.payments.maxAddresses || (global.config.payments.maxTransactionAmount && commandAmount >= global.config.payments.maxTransactionAmount) || with_payment_id) {
                    commandIndex++;
                    addresses = 0;
                    commandAmount = 0;
                }
                ...
                apiInterfaces.rpcWallet('transfer', transferCmd.rpc, function(error, result){
                    ...
                    for (var i = 0; i < transferCmd.rpc.destinations.length; i++){
                        ...
                        if (transferCmd.rpc.payment_id){
                            destination.address += global.config.poolServer.paymentId.addressSeparator + transferCmd.rpc.payment_id;
                        }
                        ...
                        redisClient.hget(global.config.coin + ':notifications', destination.address, function(error, email) {
