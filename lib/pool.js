...
var shareTrustEnabled = global.config.poolServer.shareTrust && global.config.poolServer.shareTrust.enabled;
var shareTrustStepFloat = shareTrustEnabled ? global.config.poolServer.shareTrust.stepDown / 100 : 0;
var shareTrustMinFloat = shareTrustEnabled ? global.config.poolServer.shareTrust.min / 100 : 0;
...
function processShare(miner, job, blockTemplate, nonce, resultHash,sendReply){
	...
	//stellite's variant
        ...
        hash = multiHashing.cryptonight_variant(convertedBlob, cn_variant);
	//is stellite's variant
	if (hash.toString('hex') === resultHash) {
		shareType = 'valid';
	
        } else {
	    var cn_variant = convertedBlob[0] >= 7 ? convertedBlob[0] - 6 : false;
            hash = multiHashing.cryptonight(convertedBlob, cn_variant);	
	    shareType = "invalid";
	    if(hash.toString("hex") === resultHash){
		sendReply("Incorrect monero variant. Visit https://xtlpool.com/#getting_started to get you started.");
        	log('warn', logSystem, 'Bad variant from miner %s@%s', [miner.login, miner.ip]);
		return false;
	    }else{
		sendReply("Incorrect algorithm.. Visit https://xtlpool.com/#getting_started to get you started.");
