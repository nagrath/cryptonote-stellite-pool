...
function processShare(miner, job, blockTemplate, nonce, resultHash){
	...
	//stellite's variant
        var cvn = convertedBlob[0] >= 3 ? convertedBlob[0] - 2 : false;
        hash = cryptonight_variant(convertedBlob, cnv);
	
    }

    if (hash.toString('hex') !== resultHash) {
        var cnv = convertedBlob[0] >= 3 ? convertedBlob[0] - 2 : 0;
        hash = cryptonight_v4(convertedBlob, cn_variant);
