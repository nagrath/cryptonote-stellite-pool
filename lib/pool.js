...
var cryptonight = require('cryptonight-hashing')['cryptonight'];
...
function processShare(miner, job, blockTemplate, nonce, resultHash,sendReply){
	...
	//stellite's variant
        var cn_variant = convertedBlob[0] >= 3 ? convertedBlob[0] - 2 : 0;
    ...
    if (hash.toString('hex') !== resultHash) {
		...
		hash = cryptonight(convertedBlob, cn_variant);
		if(hash.toString("hex") === resultHash){
	          sendReply("Incorrect monero variant. Stellite uses variant 2.");
		      log('warn', logSystem, 'Bad variant from miner %s@%s', [miner.login, miner.ip]);
		      return false;
		}else{
		      sendReply("Incorrect algorithm.");
		      log('warn', logSystem, 'Bad algorithm from miner %s@%s', [miner.login, miner.ip]);
		      return false;
    	}
		return false; 
